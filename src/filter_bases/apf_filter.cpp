...
APFFilter::APFFilter(int numParts, APFResampStyle resampTechnique, unsigned int pathLength) 
    : m_now(0), m_logLastCondLike(0.0), m_numParts(numParts), 
...
void APFFilter::filterOrSmooth(const Vec &data, const std::vector<std::function<const Mat(const Vec&)> >& fs)
{
    ...
    int now      = ((m_pathLength == 0) ? 0 : m_now);    
    ...
    if(m_now == 0) // first time doesn't change much from SISR
    {
        ...
        for(unsigned int ii = 0; ii < m_numParts; ++ii)
        {
            ...
            currentLogWtAdjs[ii]  = logMuEv(m_particles[now][ii]);
            currentLogWtAdjs[ii] += logGEv(data, m_particles[now][ii]);
            currentLogWtAdjs[ii] -= logQ1Ev(m_particles[now][ii], data);
        ...
        for(unsigned int ii = 0; ii < m_numParts; ++ii)   
        {
            ...
            newSamps[ii]         = fSamp(xtm1k); 
            Vec muT              = propMu(xtm1k); 
            logCurrWeightAdj[ii] = logGEv(data, newSamps[ii]) - logGEv(data, muT);
