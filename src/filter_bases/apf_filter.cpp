    ...
    if(m_now == 0) // first time doesn't change much from SISR
    {
        ...
        for(auto & h : fs){
            ...
            for(size_t prtcl = 0; prtcl < m_numParts; ++prtcl){ // iterate over all particles
                m_expectations[fId] += h(m_particles[now][prtcl]) * std::exp(m_logUnNormWeights[prtcl] - max);
        ...
        for(auto & h : fs){
            weightNormConst = 0.0;
            for(size_t prtcl = 0; prtcl < m_numParts; ++prtcl){ // iterate over all particles
                m_expectations[fId] += h(m_particles[now][prtcl]) * std::exp(m_logUnNormWeights[prtcl] - m1);
...
void APFFilter::smooth(const Vec &data, const std::vector<std::function<const Mat(const Vec&)> >& fs)
{
    ...
    // TODO this is broken after I changed filter() to handle log-sum-exp stuff
